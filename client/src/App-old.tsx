...
import { addProductToCart, addProductToWishlist, searchProducts } from "./api";
import {
  ...
  const handleToolAction = async (action: UIActionResult) => {
    console.log("UI action:", action);
     ...
     if (action.type !== "tool") {
      ...
      const fromPrompt = String(params.fromPrompt ?? "");

      if (!productId) return { status: "ignored" as const };

      try {
        if (toolName === "addToCart") {
          // optimistic UI update
          setCart((prev) => [...prev, { productId, fromPrompt }]);

          // persist in DB
          const res = await addProductToCart({ productId, fromPrompt });

          // optional: sync with DB reality
          // setCart(res.cart.map((row) => ({
          //   productId: row.product_id,
          //   fromPrompt: row.from_prompt ?? "",
          // })));
        }

        if (toolName === "addToWishlist") {
          // optimistic UI update
          setWishlist((prev) => [...prev, { productId, fromPrompt }]);

          // persist in DB
          const res = await addProductToWishlist({ productId, fromPrompt });

          // optional: sync with DB reality
          // setWishlist(res.wishlist.map((row) => ({
          //   productId: row.product_id,
          //   fromPrompt: row.from_prompt ?? "",
          // })));
        }
      } catch (err) {
        console.error("Failed to handle tool action:", err);
        // You can also rollback optimistic update if needed
