...
const App: React.FC = () => {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    // whenever we search, stay on search page
    setView("search");
  ...
  const handleToolAction = async (action: UIActionResult) => {
    ...
    try {
      if (toolName === "addToCart") {
        ...
        // persist in DB
        await addProductToCart({ productId, fromPrompt });
      }
      ...
      if (toolName === "addToWishlist") {
        ...
        // persist in DB
        await addProductToWishlist({ productId, fromPrompt });
      }
    } catch (err) {
    ...
    >
      ...
      >
        {view === "search" ? (
          <>
            ...
            >
              <div>
                ...
                >
                  Search with natural language and let the AI render your
                  shopping UI.
                </p>
              </div>
          ...
          // CART VIEW
          <CartPage onBack={() => setView("search")} />
        )}
      </div>
    </div>
  );
